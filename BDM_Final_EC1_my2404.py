...
def dis_number(col1,col2):
  list_res = col1 * [col2]
  return list_res

dis_numberUdf = F.udf(dis_number,T.ArrayType(T.DoubleType()))
num_dis_res = distance_res.withColumn('num_distance',dis_numberUdf('people_number','weighted_distance')).select(['date','poi_cbg','num_distance']).cache()

df_collect_dis = num_dis_res.groupby('date','poi_cbg').agg(
    F.collect_list('num_distance')).withColumnRenamed('collect_list(num_distance)','distance_list').cache()
...
medianUdf = F.udf(get_median,T.DoubleType())
dis_median = df_collect_dis.withColumn('median',medianUdf('distance_list'))
grouped_median = dis_median.groupBy("poi_cbg").pivot('date').sum('median').withColumnRenamed('poi_cbg','cbg_fips').sort('cbg_fips').write.option("header",True).csv(sys.argv[1])
