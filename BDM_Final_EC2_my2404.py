...
##Read the files
df_core = spark.read.option("header","true").option("escape", "\"").csv("/tmp/bdm/core-places-nyc.csv")
...
df_core = df_core.select('placekey','naics_code').cache()
df_core = df_core.filter(df_core.naics_code.startswith('4451')).cache()

placeList1 = df_core.select("placekey").rdd.flatMap(lambda x: x).collect()

#filter the placeid showed in core-places-nyc.csv
df_pattern = df_pattern.filter(F.col("placekey").isin(placeList1)).select("placekey","poi_cbg","visitor_home_cbgs", date_format("date_range_start", "yyyy-MM").alias("Formatted_start_date"),date_format("date_range_end", "yyyy-MM").alias("Formatted_end_date")).cache()
